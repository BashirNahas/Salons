    ...
    <div class="lg:col-span-2">
        @if ($blockedDates->isEmpty())
            <x-empty-state :title="__('No blocked dates')" :description="__('Block holidays or days off so customers cannot book them.')">
                <x-slot:icon>
                    <svg aria-hidden="true" class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>
                </x-slot:icon>
            </x-empty-state>
        @else
                ...
                <table class="min-w-full divide-y divide-gray-100 text-sm">
                    <thead>
                        <tr class="text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                            <th scope="col" class="px-5 py-3 text-start">{{ __('Date') }}</th>
                            <th scope="col" class="px-5 py-3 text-start">{{ __('Reason') }}</th>
                        ...
                        @foreach ($blockedDates as $blocked)
                            <tr class="transition hover:bg-gray-50/60">
                                ...
                                <td class="px-5 py-3.5 text-end">
                                    <form method="POST" action="{{ route('dashboard.blocked-dates.destroy', $blocked) }}" data-confirm="{{ __('Unblock this date?') }}">
                                        @csrf
                                        @method('DELETE')
