...
@if ($services->isEmpty())
    <x-empty-state :title="__('No services yet')" :description="__('Add your first service so customers can start booking online.')">
        <x-slot:icon>
            <svg aria-hidden="true" class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
        </x-slot:icon>
        <x-slot:action>
        ...
        <table class="min-w-full divide-y divide-gray-100 text-sm">
            <thead>
                <tr class="text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                    <th scope="col" class="px-5 py-3 text-start">{{ __('Name') }}</th>
                    <th scope="col" class="px-5 py-3 text-start">{{ __('Duration') }}</th>
                    <th scope="col" class="px-5 py-3 text-start">{{ __('Price') }}</th>
                    ...
                    <tr class="transition hover:bg-gray-50/60">
                        ...
                        <td class="px-5 py-3.5 text-end">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form method="POST" action="{{ route('dashboard.services.destroy', $service) }}" data-confirm="{{ __('Delete this service?') }}">
                                    @csrf
                                    @method('DELETE')
