...
@section('title', 'Edit Regular Customer')
@section('page-title', 'Edit Regular Customer')

@section('dashboard-content')

<div class="mx-auto max-w-xl">
    <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
        <p class="mb-5 text-sm text-gray-500">Changing the day, time, service, or staff will regenerate all upcoming appointments for this customer.</p>

        <form method="POST" action="{{ route('dashboard.recurring-bookings.update', $recurringBooking) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="customer_name" value="{{ old('customer_name', $recurringBooking->customer_name) }}" required
                ...
                <div>
                    ...
                    <input type="tel" dir="ltr" name="customer_phone" value="{{ old('customer_phone', $recurringBooking->customer_phone) }}" required
                ...
                <div>
                    ...
                    <select name="service_id" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                        @foreach($services as $service)
                            <option value="{{ $service->id }}" {{ old('service_id', $recurringBooking->service_id) == $service->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="employee_id" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                            ...
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}" {{ old('employee_id', $recurringBooking->employee_id) == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
                ...
                <div>
                    ...
                    <select name="day_of_week" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                        @foreach(\App\Models\WorkingHour::DAYS as $num => $label)
                            <option value="{{ $num }}" {{ old('day_of_week', $recurringBooking->day_of_week) == $num ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="time" name="time" value="{{ old('time', $recurringBooking->time->format('H:i')) }}" required
                           class="w-full rounded-xl border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                ...
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', $recurringBooking->start_date->toDateString()) }}" required
                           class="w-full rounded-xl border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date', $recurringBooking->end_date?->toDateString()) }}"
                           class="w-full rounded-xl border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
            ...
            <div>
                ...
                <textarea name="notes" rows="2" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">{{ old('notes', $recurringBooking->notes) }}</textarea>
            </div>

            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $recurringBooking->is_active) ? 'checked' : '' }}
