...
@if ($recurringBookings->isEmpty())
    <x-empty-state :title="__('No regular customers yet')" :description="__('Add a customer with a standing weekly appointment (e.g. every Saturday at 10:00).')">
        <x-slot:icon>
            <svg aria-hidden="true" class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
        </x-slot:icon>
        <x-slot:action>
        ...
        <table class="min-w-full divide-y divide-gray-100 text-sm">
            <thead>
                <tr class="text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                    ...
                    <th scope="col" class="px-5 py-3 text-start">{{ __('Day & Time') }}</th>
                        ...
                        <td class="px-5 py-3.5 text-end">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form method="POST" action="{{ route('dashboard.recurring-bookings.destroy', $rb) }}"
                                      data-confirm="{{ __('Cancel this recurring booking and remove the upcoming appointments for :name?', ['name' => $rb->customer_name]) }}">
                                    @csrf
                                    @method('DELETE')
